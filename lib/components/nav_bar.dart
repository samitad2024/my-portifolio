...
class NavBar extends StatelessComponent {
  ...
  static final List<StyleRule> styles = [
    ...
    css('.navbar-label')
        .box(padding: EdgeInsets.only(right: 35.px))
        .text(fontSize: 12.px, decoration: TextDecoration.none)
        .flexbox(
          direction: FlexDirection.row,
          justifyContent: JustifyContent.spaceBetween,
          alignItems: AlignItems.center,
    ...
    css('.navbar-label:hover')
        .text(color: themePrimaryColor)
        .box(transition: Transition('color', duration: 500)),
    css(
      '.labels',
    ).flexbox(direction: FlexDirection.row, justifyContent: JustifyContent.end),
    css('.brand').text(fontFamily: FontFamily('Agustina'), fontSize: 24.px),
  ];
}
