...
class BasicInfoSection extends StatelessComponent {
  ...
  static final List<StyleRule> styles = [
    css('.info-section')
        ...
        .box(padding: EdgeInsets.only(left: 10.vw)),
    css('.welcome')
        .box(margin: EdgeInsets.only(bottom: 2.5.vh))
        .flexbox(alignItems: AlignItems.end),
    css('.welcome-text').box(
      padding: EdgeInsets.only(right: 12.px, bottom: 2.px),
    ),
    css('.welcome-text', [
      css.text(fontFamily: FontFamily('Montserrat'), fontSize: 20.px),
    ]),
    css('.first-name').box(padding: EdgeInsets.only(top: 3.vh)),
    css('.first-name', [
      css.text(
    ...
    css('.main-image')
        .box(height: 85.vh, width: 75.vh)
        .background(
          image: ImageStyle.url(StaticAssets.bwImage),
          size: BackgroundSize.cover,
        ),
    css('.play-icon', [css.text(color: themePrimaryColor)]),
    css(
      '.subtitle',
    ).flexbox(direction: FlexDirection.row, alignItems: AlignItems.center),
    css('.dynamic-subtitles').box(margin: EdgeInsets.only(left: 15.px)),
    css('.dynamic-subtitles', [css.text(fontSize: 18.px)]),
    css('.socials')
        .box(padding: EdgeInsets.only(top: 2.vh))
        .flexbox(direction: FlexDirection.row, alignItems: AlignItems.center),
    css('.social-icon').box(padding: EdgeInsets.only(right: 25.px)),
    css('.social-icon', [css.text(fontSize: 25.px, color: Colors.white)]),
    css(
      '.social-icon:hover',
    ).box(transition: Transition('color', duration: 300)),
    css('.social-icon:hover', [css.text(color: themePrimaryColor)]),

    /// MEDIA QUERY 1100px
    css.media(MediaQuery.screen(maxWidth: 1100.px), [
      css('.welcome-text', [css.text(fontSize: 15.px)]),
      css('.first-name', [css.text(fontSize: 50.px)]),
      css('.last-name', [css.text(fontSize: 50.px)]),
      css('.dynamic-subtitles', [css.text(fontSize: 14.px)]),
    ...
    css.media(MediaQuery.screen(maxWidth: 786.px), [
      ...
      css(
        '.main-image',
      ).box(height: 55.vh, width: 45.vh).flexItem(alignSelf: AlignSelf.end),
      css('.info').flexItem(alignSelf: AlignSelf.start),
    ]),
  ];
}
